...
async def get_narrative(event_id: str):
    ...
    Checks PostgreSQL first, then SQLite buffer when PostgreSQL is unavailable.
    """
    try:
        event = None
        ...
        try:
            ...
            logger.warning(
                f"PostgreSQL unavailable for narrative lookup: {db_error}"
            )

        # Fall back to locally buffered events
        if event is None:
            buffered_events = event_buffer.get_all()

            for buffered_event in buffered_events:
                if buffered_event.event_id == event_id:
                    event = buffered_event.dict()
                    break
