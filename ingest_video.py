...
def generate_annotated_video(
    ...
    # Transcode to 100% web-standard H.264 MP4 with faststart for seamless HTML5 video playback
    import shutil
    import subprocess
    ffmpeg_exe = shutil.which("ffmpeg")
    if ffmpeg_exe and out_video_path.exists():
        temp_web_path = out_video_path.with_name(f"temp_web_{out_video_path.name}")
        try:
            cmd = [
                ffmpeg_exe, "-y",
            ...
            if res.returncode == 0 and temp_web_path.exists() and temp_web_path.stat().st_size > 0:
                temp_web_path.replace(out_video_path)
                print(f"Transcoded annotated video to browser-standard H.264 MP4 ({out_video_path.stat().st_size} bytes)")
        except Exception as e:
            print(f"Notice: ffmpeg transcode note: {e}")
            if temp_web_path.exists():
                temp_web_path.unlink(missing_ok=True)
...
def create_auto_calibration(
    ...
    return Calibration(
        video_id=video_id,
        H=H_auto,
        location=location if location else [12.86889, 74.86389],
...
def run_pipeline_on_video(
    ...
    if calib_path and calib_path.exists():
        ...
        if location:
            calib.location = location
        calib_type_str = f"Custom Calibration ({calib.video_id})"
    else:
        calib = create_auto_calibration(video_path, location=location)
